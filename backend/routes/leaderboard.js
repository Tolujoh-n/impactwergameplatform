  ...
  try {
    ...
    for (const user of users) {
      ...
      if (type === 'free') {
        ...
        user.points = marketPredictions.reduce((sum, p) => sum + (p.payout || 0), 0);
      } else {
        ...
        const boostPayouts = boostPredictions.reduce((sum, p) => sum + (p.payout || 0), 0);
        const marketPayouts = marketPredictions.reduce((sum, p) => sum + (p.payout || 0), 0);
